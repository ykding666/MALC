...
class RandomMultipleGallerySampler(Sampler):
    def __init__(self, data_source, num_instances=16):
        ...
        self.pid_clothid = defaultdict(list)

        self.pid_index = defaultdict(list)
        self.num_instances = num_instances

        for index, (_, pid, cam, clothid) in enumerate(data_source):
            if pid < 0:
                continue
            self.index_pid[index] = pid
            self.pid_cam[pid].append(cam)
            self.pid_clothid[pid].append(clothid)
    ...
    def __iter__(self):
        ...
        for kid in indices:
            ...
            _, i_pid, i_cam, i_clothid = self.data_source[i]
